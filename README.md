...
2. Download and install [PostgreSQL](https://www.postgresql.org/download/).
    - Install PostgreSQL on `<xampp root folder>\pgsql\<postgresql version number>`. For example, in this document, XAMPP is installed on `C:\xampp`. Thus PostgreSQL's installation directory would be `C:\xampp\pgsql\13.3\`. Note the directory where PostgreSQL is install, it will be needed later.
...
3. Open the `php.ini` file located at `<xampp root folder>\php\` using a text editor and uncomment the lines below.
    >extension=pdo_pgsql   
    >extension=pgsql
    - If there are no exact lines, uncomment the extension lines with the words `pdo_pgsql` and `pgsql`.
...
9. Done! To check the Postgres and the phppgadmin installation, launch XAMPP and start the Apache module. The installation is at `http://localhost/phppgadmin/`.
